...
namespace nilnul.num.quotient._continuedFrac
{
		...
		
	{
		...
		static public SimpleProper Create(IEnumerable<Positive1> denominators) {
			var count = denominators.Count();

			if (count ==1)
			{
				return new SimpleProper(denominators.First());
			}
			if (count==2)
			{
				return new SimpleProper( _TransformPlural(denominators.ToArray()));

			}

			return new SimpleProper();
		}
		...
		public SimpleProper(params Positive1[] denominators):this(denominators as IEnumerable<Positive1>)
		{
		}


		public SimpleProper(IEnumerable<Positive1> denominators)
		{
			Assert(denominators);
