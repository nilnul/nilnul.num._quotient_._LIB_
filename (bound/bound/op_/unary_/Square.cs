...
namespace nilnul.num.quotient.bound.op_.unary_
{
	static public class _SqureX
	{
		static public R1 Op_assumeContainZero( R1 range) {
			...
			var lowerAbs = nilnul.num.quotient.border.op_.unary_.Abs.Singleton.op(range.lower);

			var upperAbs = nilnul.num.quotient.border.op_.unary_.Abs.Singleton.op(range.upper);
			var max = nilnul.num.quotient.border_.upper.op_.binary_._MaxX.Max(
				lowerAbs
				,
				upperAbs
			);
			...
			return new R1(
					...
					, 
					nilnul.num.quotient.border.op_.unary_.Square.Singleton.op(max)
			);
		}
		...
		static public R1 Op_assumePositive( R1 range) {
			...
			return new R1(
					nilnul.num.quotient.border.op_.unary_.Square.Singleton.op(range.lower)
					, 
					nilnul.num.quotient.border.op_.unary_.Square.Singleton.op(range.upper)
			);
		}
